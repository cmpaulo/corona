...
ax.text(len(psum)-5,psum.values.max(),"Sum SouthAmerica",color="g")
ax.text(len(p)-15,1,'update on '+str(datetime.now().date()))
ax.set_yscale('log')
ax.set_title("Countries of South America")
ax.set_ylabel("Number of Infections")
ax.set_xlim(0,len(p)+15)
...
ax.text(len(p)-5,20,'update on '+str(today))
ax.set_yscale('log')
...
ax.text(len(p)-10,50,'update on '+str(datetime.now().date()))
ax.set_yscale('log')
